...
class GpsWpCommander(Node):
    ...
    def start_wpf(self):
        ...
        for wp in wps:
            ...
            log = 'long{:f}, lat={:f}, alt={:f}'.format(self.req.ll_point.longitude, self.req.ll_point.latitude,
            ...
            log = 'x={:f}, y={:f}, z={:f}'.format(self.future.result().map_point.x, self.future.result().map_point.y,
...
def main():
    ...
    default_yaml_file_path = os.path.join(get_package_share_directory("agri_bot"), "config", "gps_waypoints.yaml")
    if len(sys.argv) > 1:
